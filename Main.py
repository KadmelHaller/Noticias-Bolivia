...
# Keywords optimizadas para máxima captura
KEYWORDS = [
...
def scraping_avanzado(fuentes):
    ...
    for i, f in enumerate(fuentes):
        st.write(f"📡 Capturando: {f['n']}...")
        pb.progress((i + 1) / len(fuentes))
        try:
            r = requests.get(f['u'], headers=headers, timeout=15)
            soup = BeautifulSoup(r.text, 'html.parser')
            
            # Buscamos en todas las etiquetas de texto probables
            ...
            for el in elementos:
                ...
                # Verificación de relevancia y longitud
                if len(tit) > 22 and any(k in (tit + url).lower() for k in KEYWORDS):
                    if tit not in vistos:
                        datos += f"FUENTE: {f['n']} | CIUDAD: {f['r']} | INFO: {tit} | URL: {url}\n\n"
                        vistos.add(tit)
        except: continue
    return datos

def procesar_ia(datos_raw, contexto):
    try:
        ...
        PRIORIDAD: 1. IMPUESTOS (unificar), 2. GOBIERNO, 3. ECONOMÍA.

        ORGANIZACIÓN: 
        1. COCHABAMBA (Incluir medios locales y noticias de TV que mencionen Cbba).
        2. SANTA CRUZ (Incluir medios locales, La Voz Digital, Visión 360 y TV sobre SCZ).

        FORMATO ESTRICTO:
        *TITULAR EN MAYÚSCULAS*
        NOMBRE DEL MEDIO EN MAYÚSCULAS
        Resumen de 5 líneas analizando el impacto. Sin etiquetas como 'Resumen:' o 'Impacto:'.
        Enlace directo aquí.

        REGLA VISUAL: Solo UN asterisco (*) antes y después del titular. El nombre del medio en la línea de abajo. Prohibido usar doble asterisco (**).
        """
...
if api_key:
    ...
    with c1:
        if st.button('🚀 FASE 1: PRENSA, TV Y DIGITALES'):
            medios = [
                ...
                # TV (ORGANIZADO POR IA SEGÚN CONTENIDO)
            ...
            datos_t = scraping_avanzado(medios)
            if datos_t:
                st.session_state.reporte_medios = procesar_ia(datos_t, "Prensa y TV")
            else:
                st.warning("No se detectaron noticias. Verifica la conexión.")

    with c2:
        if st.button('📱 FASE 2: REDES SOCIALES (GOOGLE RELEVAMIENTO)'):
            # Búsqueda en Google para saltar muros de login en RRSS
            rrss_sources = [
            ...
            datos_r = scraping_avanzado(rrss_sources)
            if datos_r:
                st.session_state.reporte_rrss = procesar_ia(datos_r, "Redes Sociales")

    # --- DESPLIEGUE FINAL ---
    if st.session_state.reporte_medios:
        st.markdown("### 📰 REPORTE PRENSA, TV Y DIGITAL")
    ...
    st.info("Ingresa tu API Key en la barra lateral para iniciar el monitoreo.")
