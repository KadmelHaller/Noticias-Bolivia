    ...
    for i, fuente in enumerate(fuentes):
        ...
        try:
            ...
            for l in links:
                ...
                if len(l.get_text().strip()) < 30 or url_n in vistos or any(x in url_n for x in ['/tag/', '/category/']): continue
                try:
...
if api_key:
    ...
    if st.button('🚀 GENERAR REPORTE'):
        ...
        if len(raw_data) > 300:
            try:
                # CAMBIO CLAVE: Usando el modelo con mayor cuota disponible en tu lista
                model = genai.GenerativeModel('models/gemini-flash-latest')
                
                prompt = f"""
                HOY ES: {fecha_hoy_bonita}.
                1. FILTRAR: Economía, Gobierno, Aduana, Impuestos y Gestión Municipal.
                2. EXCLUIR: Deportes, Farándula, Policiales, Internacional.
                3. HORA: Usa 'HORA'. Si está vacía, estima una de hoy y pon "(aprox.)".
                4. FORMATO: Lista continua, *TITULAR*, MEDIO, HH:MM, Resumen (4-6 líneas), URL.
                """
                ...
                st.error(f"Error de Cuota/IA: {str(e)}")
